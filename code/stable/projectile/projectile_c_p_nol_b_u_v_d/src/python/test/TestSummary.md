...
when it satisfies all constraints if it is entered using floating point
numbers. These values are the values from the default `input.txt` generated by
Drasil.
...
**Rationale:** The InputParameters module should be able to read data correctly
when it satisfies all constraints if it is entered using integers. These values
are the values from the default `input.txt` generated by Drasil rounded to the
nearest integer.
...
These values are the values from the default `input.txt` generated by Drasil,
with $p_\text{target}$ reduced by $1.0 \text{m}$ so that the projectile goes
long.
...
point numbers. These values are the values from the default `input.txt`
generated by Drasil.
...
correctly when it satisfies all constraints if it is entered using floating
point numbers. These values are the values from the default `input.txt`
generated by Drasil, with $p_\text{target}$ reduced by $1.0 \text{m}$ so that
the projectile goes long.
...
These values are the values from the default `input.txt` generated by Drasil
with $v_\text{launch}$ set to $0.0 \frac{\text{m}}{\text{s}}$ so that this
...
**Rationale:** The InputParameters module should be able to verify data
correctly when it violates the lower bound constraint on $\theta$. These values
are the values from the default `input.txt` generated by Drasil rounded to the
nearest integer with $\theta$ set to $0 \text{ rad}$ so that this constraint is
violated.
...
**Rationale:** The InputParameters module should be able to verify data
correctly when it violates the upper bound constraint on $\theta$. These values
are the values from the default `input.txt` generated by Drasil rounded to the
nearest integer with $\theta$ set to $2 \text{ rad}$ so that this constraint is
violated.
...
These values are the values from the default `input.txt` generated by Drasil
with $p_\text{target}$ set to $0.0 \text{m}$ so that this constraint is
violated.
