...
def test_func_t_flight_valid(filename, t_flight):
    assert isclose(Calculations.func_t_flight(conftest.inParams[filename], valid_g),
                       t_flight)
...
def test_func_p_land_valid(filename, p_land):
    assert isclose(Calculations.func_p_land(conftest.inParams[filename], valid_g),
                       p_land)
...
def test_func_d_offset_valid(filename, d_offset):
    assert isclose(Calculations.func_d_offset(
        conftest.inParams[filename], Calculations.func_p_land(
            conftest.inParams[filename], valid_g)), d_offset)
...
def test_func_t_flight_neg_g(filename, t_flight):
    assert isclose(Calculations.func_t_flight(conftest.inParams[filename], neg_g),
                       -t_flight)

# \brief Tests calculation of p_land with negative gravitational acceleration
@mark.parametrize("filename,p_land", get_expected("p_land"))
def test_func_p_land_neg_g(filename, p_land):
    assert isclose(Calculations.func_p_land(conftest.inParams[filename], neg_g),
                       -p_land)
...
def test_func_d_offset_neg_g(filename, d_offset,p_target):
    assert isclose(Calculations.func_d_offset(
        conftest.inParams[filename], Calculations.func_p_land(
            conftest.inParams[filename], neg_g)), -d_offset - 2 * p_target)
