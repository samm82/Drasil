...
intLinProg = tm
  (ilpModel "canonIntLinProg" (cn' "canonical integer linear program")
    $ maxILP xVec cVec
      $ sy aMat `mulRe` sy xVec $<= sy bVec NE.:| [sy xVec $>= sy zeroVec, isIn (sy xVec) (Vect Nothing Integer)])
  ...
  [sy cVec `mulRe` sy xVec,
   ...
   sy xVec $>= sy zeroVec,
   isIn (sy xVec) (Vect Nothing Integer)] -- FIXME: apparently this is needed since generation doesn't happen from ModelKinds
  []
  [dRef ilpWiki]
