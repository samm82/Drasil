...
import Database.Drasil (HasUID(uid), HasChunkRefs (chunkRefs))
...
instance HasUID        DifferentialModel where uid = uid . (^. conc)
-- | Equal if 'UID's are equal.
instance Eq            DifferentialModel where a == b = uid a == uid b
...
instance HasChunkRefs  DifferentialModel where chunkRefs = cdom . view conc -- FIXME: placeholder, (but it's not entirely wrong!).
